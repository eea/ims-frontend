...
class Github(object):
    ...
    def __init__(self,
        ...
        sources="https://raw.githubusercontent.com/eea/ims-frontend/develop/jsconfig.json",
        timeout=15,
    ...
    def check_pulls(self, repo):
        ...
        try:
            with contextlib.closing(urllib.request.urlopen(
                ...
                for pull in pulls:
                    self.status = 1
                    self.logger.warning("%s - %s - %s - %s from %s", name, pull.get('user').get('login', '-'), pull.get('title', '-'), pull.get('html_url', '-'), pull.get('created_at', '-') )
    ...
    def start(self):
       ...
       """
       self.repos = []
       with contextlib.closing(urllib.request.urlopen(self.sources, timeout=self.timeout)) as conn:
